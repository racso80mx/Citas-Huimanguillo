...
import { Trash2, Pencil, Baby, ShieldPlus, Loader2, ArrowUpDown, ArrowUp, ArrowDown, FileDown, ClipboardCopy, MessageCircle, ChevronDown, RefreshCw } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { updateAppointmentStatus, rescheduleAppointment, cloneAppointment, getAnnouncements, getModuleSettings, getAvailableSlotsForDate } from '@/lib/actions';
...
export function VaccineAppointmentList({ appointments, isAdmin = false, onDelete, onEditSuccess }: VaccineAppointmentListProps) {
  ...
  useEffect(() => {
    if (reschedulingAppointment && newDate) {
        startFetchingSlots(async () => {
            const data = await getAvailableSlotsForDate('vaccine', newDate.toISOString());
  ...
  const handleWhatsApp = (app: VaccineAppointment) => {
    ...
    const message = encodeURIComponent(`Hola ${app.patient.name}, le informamos que su cita de vacunación ha sido reagendada para el día ${formattedDate} a las ${app.time} hrs.`);
  ...
  const handleStatusChange = (appointmentId: string, status: AppointmentStatus) => {
    startUpdateTransition(async () => {
      await updateAppointmentStatus(appointmentId, status, 'vaccine');
      onEditSuccess?.();
    });
  };
  ...
  const handleRescheduleConfirm = () => {
    ...
    startRescheduleTransition(async () => {
        const result = await rescheduleAppointment(reschedulingAppointment.id, newDate.toISOString(), 'vaccine', newTime);
        if (result.success) {
            toast({ title: 'Cita Reprogramada' });
            const whatsappEnabled = isAdmin ? settings?.archivoWhatsAppEnabled : settings?.vacunasWhatsAppEnabled;
            if (whatsappEnabled && reschedulingAppointment.patient?.phoneNumber) {
                ...
                const msg = encodeURIComponent(`Hola ${reschedulingAppointment.patient.name}, le informamos que su cita de vacunación del día ${oldDate} ha sido reagendada. Nueva cita: ${newDateFormatted} a las ${newTime} hrs.`);
  ...
  if (!appointments || appointments.length === 0) return <div className="text-center py-10 opacity-60">No hay citas de vacunación.</div>;
        ...
        <TableBody>
          {appointments.map((app) => (
            <TableRow key={app.id}>
              ...
              <TableCell>
                 <Tooltip>
                    <TooltipTrigger asChild><Button variant="ghost" size="icon"><ShieldPlus className="h-4 w-4" /></Button></TooltipTrigger>
                    <TooltipContent><p className='font-bold mb-2'>Vacunas:</p><ul className='list-disc pl-4'>{app.vaccines.map(v => <li key={v.id}>{v.name}</li>)}</ul></TooltipContent>
                </Tooltip>
              </TableCell>
               {isAdmin && (
