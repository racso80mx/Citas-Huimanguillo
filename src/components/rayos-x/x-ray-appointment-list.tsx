...
import { updateAppointmentStatus, rescheduleAppointment, cloneAppointment, getAnnouncements, getXRayStudies, getModuleSettings, getAvailableSlotsForDate } from '@/lib/actions';
...
export function XRayAppointmentList({ appointments, isAdmin = false, onDelete, onEditSuccess }: XRayAppointmentListProps) {
  ...
  useEffect(() => {
    if (reschedulingAppointment && newDate) {
        startFetchingSlots(async () => {
            const data = await getAvailableSlotsForDate('x-ray', newDate.toISOString());
  ...
  const handleWhatsApp = (app: XRayAppointment) => {
    ...
    const message = encodeURIComponent(`Hola ${app.patient.name}, le informamos que su cita de Rayos X con folio ${app.appointmentNumber} ha sido reagendada para el día ${formattedDate} a las ${app.time} hrs. Estudio: ${app.studyName}.`);
  ...
  const handleStatusChange = (appointmentId: string, status: AppointmentStatus) => {
    startUpdateTransition(async () => {
      await updateAppointmentStatus(appointmentId, status, 'xray');
      onEditSuccess?.();
    });
  };
  ...
  const handleRescheduleConfirm = () => {
    ...
    startRescheduleTransition(async () => {
        const result = await rescheduleAppointment(reschedulingAppointment.id, newDate.toISOString(), 'xray', newTime);
        if (result.success) {
            toast({ title: 'Cita Reprogramada' });
            const whatsappEnabled = isAdmin ? settings?.archivoWhatsAppEnabled : settings?.rayosXWhatsAppEnabled;
            if (whatsappEnabled && reschedulingAppointment.patient?.phoneNumber) {
                ...
                const msg = encodeURIComponent(`Hola ${reschedulingAppointment.patient.name}, le informamos que su cita de Rayos X del día ${oldDate} ha sido reagendada. Nueva cita: ${newDateFormatted} a las ${newTime} hrs. Estudio: ${reschedulingAppointment.studyName}.`);
  ...
  if (!appointments || appointments.length === 0) return <div className="text-center py-10 opacity-60">No hay citas de Rayos X.</div>;
      ...
      <Table>
        ...
        <TableBody>
          {appointments.map((app) => (
            <TableRow key={app.id}>
              ...
              <TableCell className="text-xs uppercase">{app.studyName}</TableCell>
               {isAdmin && (
       ...
       {reschedulingAppointment && (
        <Dialog open={!!reschedulingAppointment} onOpenChange={(o) => !o && setReschedulingAppointment(null)}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Reprogramar Rayos X</DialogTitle>
