...
function fuzzyMapInsumo(item: any) {
    ...
    const findValue = (options: string[]) => {
        // PRIORIDAD ABSOLUTA AL CAMPO "FECHA CADUCIDAD" (Exacto del usuario)
...
export async function createPrescription(p: any) { const id = uuidv4(); const f = `REC-${Math.floor(1000 + Math.random() * 9000)}`; const data = { ...p, id, folio: f, status: 'pendiente', createdAt: new Date().toISOString(), expiresAt: addDays(new Date(), 1).toISOString() }; await setDoc(doc(adminDb, 'prescriptions', id), data); return { success: true, folio: f, prescription: data }; }
...
export async function downloadBackupAction() { const [p, c, a] = await Promise.all([getRawCollection('patients', 2000), getRawCollection('clinics', 100), getRawCollection('appointments', 1000)]); return { success: true, data: { patients: p, clinics: c, appointments: a, labAppointments: [], xrayAppointments: [], ultrasoundAppointments: [], vaccineAppointments: [] } }; }
...
export async function scanDuplicates(criteria: string) { const p = await getRawCollection('patients', 1000); const g = new Map<string, Patient[]>(); p.forEach(x => { let k = criteria === 'expediente' ? String(x.expediente || '') : criteria === 'curp' ? String(x.curp || '') : `${x.name} ${x.paternalLastName}`.toUpperCase(); if (!k) return; if (!g.has(k)) g.set(k, []); g.get(k)!.push(x as Patient); }); return Array.from(g.values()).filter(x => x.length > 1); }
...
export async function getAvailableSlotsForDate(cid: string, d: string) { const dateOnly = d.split('T')[0]; const snap = await getDocs(query(collection(adminDb, 'appointments'), where('clinicId', '==', cid), where('date', '>=', dateOnly), where('date', '<=', dateOnly + 'T23:59:59'))); return { timeSlots: [], tokens: [] }; }
...
export async function getBIData() {
    const [a, l, x, u, v, c, cl] = await Promise.all([
        getRawCollection('appointments', 500),
        getRawCollection('labAppointments', 200),
        getRawCollection('xrayAppointments', 200),
        getRawCollection('ultrasoundAppointments', 200),
        getRawCollection('vaccineAppointments', 200),
        getClinicsData(),
        getRawCollection('colonias', 500)
    ]);
    return { appointments: a, labAppointments: l, xRayAppointments: x, ultrasoundAppointments: u, vaccineAppointments: v, clinics: c, colonias: cl };
}
...
export async function getConsultationsByPatientId(pid: string) {
    const q = query(collection(adminDb, 'medicalConsultations'), where('patientId', '==', pid), limit(50));
    const s = await getDocs(q);
    return s.docs.map(d => ({ ...serializeData(d.data()), id: d.id })) as MedicalConsultation[];
}
...
export async function getConsultationByAppointmentId(aid: string) {
    ...
    const s = await getDocs(q);
    return s.empty ? null : { ...serializeData(s.docs[0].data()), id: s.docs[0].id } as MedicalConsultation;
}
