...
// --- ACCIONES DE SERVIDOR SINCRONIZADAS ---
...
export async function getPatientsData(options?: any): Promise<Patient[]> {
  ...
  if (options?.searchCurp) {
    const q = query(colRef, where('curp', '==', options.searchCurp.toUpperCase().trim()), limit(5));
    const snap = await getDocs(q);
  ...
  if (options?.status && options.status !== 'Total') {
      q = query(colRef, where('status', '==', options.status), limit(100));
  } else {
...
export async function updateColonias(colonias: Colonia[]) { const batch = writeBatch(adminDb); colonias.forEach(x => batch.set(doc(adminDb, 'colonias', x.id), x)); await batch.commit(); return { success: true }; }
...
export async function downloadBackupAction() {
    const [appointments, labAppointments, xRayAppointments, ultrasoundAppointments, vaccineAppointments, patients, clinics] = await Promise.all([
        getRawCollection('appointments', 1000), getRawCollection('labAppointments', 1000), 
        getRawCollection('xrayAppointments', 1000), getRawCollection('ultrasoundAppointments', 1000), 
        getRawCollection('vaccineAppointments', 1000), getRawCollection('patients', 2000), 
        getRawCollection('clinics', 100)
    ]);
...
export async function getBIData() {
    const [appointments, labAppointments, xRayAppointments, ultrasoundAppointments, vaccineAppointments, clinics, colonias] = await Promise.all([
        ...
        getRawCollection('vaccineAppointments', 500), getClinicsData(), 
        getRawCollection('colonias', 500)
    ]);
...
export async function getAvailableSlotsForDate(cid: string, date: string) {
    ...
    const apps = await getRawCollection('appointments', 500);
    ...
    if (clinic.bookingMode === BookingMode.Token) {
        ...
        const tokens = Array.from({ length: total }, (_, i) => i + 1).filter(t => !bookedTimes.includes(`Ficha ${t}`));
        return { tokens };
    } else {
...
export async function applyStatusUpdateChunk(expedientes: string[], status: PatientStatus) {
    ...
    for (const exp of expedientes) {
        const q = query(colRef, where('expediente', '==', exp), limit(1));
        const snap = await getDocs(q);
        if (!snap.empty) { batch.update(snap.docs[0].ref, { status }); updatedCount++; }
        else {
            const withZero = exp.startsWith('0') ? exp : '0' + exp;
            const q2 = query(colRef, where('expediente', '==', withZero), limit(1));
            const snap2 = await getDocs(q2);
            if (!snap2.empty) { batch.update(snap2.docs[0].ref, { status }); updatedCount++; }
        }
    }
    await batch.commit();
...
export async function scanDuplicates(criteria: 'expediente' | 'curp' | 'name') {
    ...
    patients.forEach(p => {
        ...
        else if (criteria === 'curp') key = String(p.curp || '');
        else key = `${p.name} ${p.paternalLastName}`.toUpperCase();
        if (!key) return;
        if (!groups.has(key)) groups.set(key, []);
        groups.get(key)!.push(p as Patient);
    });
...
export async function normalizeExpedientesAction() {
    ...
    let count = 0;
    patients.forEach(p => {
        const exp = String(p.expediente || '');
        if (exp && !exp.startsWith('0')) {
            batch.update(doc(adminDb, 'patients', p.id), { expediente: '0' + exp });
            count++;
        }
    });
    await batch.commit();
...
export async function updateSpecialActionDays(items: SpecialActionDay[]) {
    ...
    items.forEach(x => batch.set(doc(adminDb, 'specialActionDays', uuidv4()), x));
    await batch.commit();
    return { success: true };
}

export async function getArchiveSettingsData() {
    const snap = await getDoc(doc(adminDb, 'module_passwords', 'archive'));
    return { password: snap.exists() ? snap.data().password : '2026' };
}
...
export async function getPharmacySettingsData() {
    const snap = await getDoc(doc(adminDb, 'module_passwords', 'pharmacy'));
    return { password: snap.exists() ? snap.data().password : 'farmacia2026' };
}
...
export async function getWarehouseSettingsData() {
    const snap = await getDoc(doc(adminDb, 'module_passwords', 'warehouse'));
    return { password: snap.exists() ? snap.data().password : 'almacen2026' };
}
...
export async function getBISettingsData() {
    const snap = await getDoc(doc(adminDb, 'module_passwords', 'bi'));
    return { password: snap.exists() ? snap.data().password : 'bi2026' };
}
...
export async function getAdminSettingsData() {
    const snap = await getDoc(doc(adminDb, 'module_passwords', 'superadmin'));
    return { password: snap.exists() ? snap.data().password : 'Hu1m4ngu1ll0' };
}
